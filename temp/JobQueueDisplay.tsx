...
const JobQueueDisplay: React.FC<JobQueueDisplayProps> = ({
	...
	const getKeyParameters = (job: Job) => {
		if (job.type === 'benchmark') {
			return `${job.securityParameter}, ${job.iterations} iterations`;
			...
			if (job.algorithm === 'grover') {
				...
				return `${job.shotCount} shots, Target: ${job.target}`;
			}
		}
		return 'N/A';
	};
						...
						<TableBody>
							{jobs.map((job) => (
								...
								>
									...
									<TableCell sx={{ color: isDarkMode ? '#FFFFFF' : '#000000' }}>
										{job.status === 'running'
											? `Run ${job.runCount || 0} of ${job.numberOfRuns}`
											: job.status === 'completed'
											? `${job.numberOfRuns} runs completed`
